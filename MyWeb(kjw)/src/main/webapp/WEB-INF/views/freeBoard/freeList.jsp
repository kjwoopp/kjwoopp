        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-9 col-xs-12 board-table">
                        ...
                        <tbody>
                        	<c:forEach var="vo" items="${boardList}" >
                        		<tr>
                                	...
                                	<td><fmt:formatDate value="${vo.updatedate}" pattern="yyyy년MM월dd일 hh:mm"/></td>
                            </tr>
                    ...
                    <!--페이지 네이션을 가져옴-->
		    	<form action="freeList"  name="pdgeForm" id="pageForm">
...
<script >
	...
	//컨트롤러에서 전달된 msg를 받아옴, 공백이 아니라면 msg출력
	window.onload=function(){
		
		//뒤로가기 실행후에 앞으로 가기 했을때 변경된 기록이 공백이라면 함수 종료
		if (history.state == '') {
			return;
		}
	...
	//등록처리
    var btn = document.getElementById("freeRegist");
    btn.onclick = function() {
    	
       location.href = "freeRegist";
    }
    
    //1.search폼 page폼 2개의 폼으로 분리
    //2.두 form에서 pageNum,amount,searchType,searchName을 동시에 전송
    //3. page폼을 a태크 -> 폼전송 방식으로 변경
    //4.ctitera에 searchType,searchName추가
    //5.게시글 쿼리, 전체게시글수 쿼리를 동적 쿼리로 변경
    //6.검색후에 검색의 정보를 select박스에 키워드 처리, input 태그에 키워드 처리
    ...
    //검색처리(페이지 클릭시 폼전송)
    function page(num) {
		event.preventDefault(); // a태그를 멈추게하는방법
		
		var pageNum = document.querySelector("#pageForm #pageNum").setAttribute("value",num); //페이지폼의 pageNum
