			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-8 section-inner">
					...
					<div class="image-inner">
						...
						<img src="view?fileLoca=20201212&fileName=ccc.jpg">
					</div>
		...
		$(document).ready(function(){
			...
			function regist(){
				...
				if (file != "jpg" && file != "png" && file != "bmp") {
					alert("이미지 파일 jpg,png,bmp 파일만 등록 가능합니다.");
					$(".fileDiv").css("display", "");
					return false;
				...
				formData.append("file",fileData[0].files[0]);//파일이름으로 파일 정보를 폼에저장
				...
				$.ajax({
					...
					type:"POST",
					data:formData,
					processData:false,//폼의 &변수 =값 형식으로 자동으로 변경되는 것을 막는요소
					contentType:false,//ajax 에서  false로 처리하면 "multipart/form-data" 형식으로 변경됨
					success: function(result){
						//success돌아오면 
						//파일 업로드 태그 값을 비우기
						//글영역태그 비우기
						//미리보기 display none을 통해 숨기기
						//getList(true)
						if (result == "success") {
							$(".fileDiv").css("display", "");
							$("#content").val("");
			...
			function getList(reset){
				//1.getjson을 통해 DB에 저장되어 있는 모든 값을 json형태로 받아온다
				//2.contentDiv 영역안에 list개수만큼 회전해서 화면에 츄가
				//3.단 화면에 띄워야 하는 이미지영역에서는 src안에 view?값&값 호출하는 코드작성
				...
				$.getJSON(
					...
					function(data){
						for (var i = 0; i < data.length; i++) {
		                    ...
		                    var hour = parseInt((time/(1000*60*60))%24)+9;
		                    ...
		                    str +="<a href='##'><i class='glyphicon glyphicon-remove'></i>삭제하기</a></div>";
